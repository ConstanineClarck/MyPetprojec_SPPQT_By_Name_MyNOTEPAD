...
void MainWIndow::onOpen()             // Слот обработки открытия существующего документа
{
    ...
    {
        QByteArray text = file.readAll();                  // Считаем сожержимое в массив байтов?
        textEdit->setText(text);                           // Передаёт указатель для управления редактированием текста
...
{
    QString text = textEdit->toPlainText();                // Получаем содержимое текста из виджета типа QTextEdit и присваиваем его переменой QString! textEdit это указатель на экземпляр QTextEdit, toPlanText() - это метод, возвращающий текстовое содержимое в формате QString, таким образом он извлекает весь текст из текщего элемента управлениятекстовым редактированием и возвращённая переменная QString присваивается переменной с именем text
    ...
    {
        QMessageBox::warning(this, "Ошибка", "Не удалось сохранить файл");     // Если проверка на открытие файла для сохранения прошла не успешно, то выводит сообщение об ошибке, что файл не был сохранён
    }
